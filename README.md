...
- **blurOnly**: _boolean_
  ...
  - Skip creating a `low-resolution` image and won't include a `<link preload ... />` tag in console output.
  - See below 👇.
- **blurSize**: _number_
...
- ⚠️ **Caution**: The tool uses `eval()` to execute the `pixx` function. `eval()` is a powerful function that can execute arbitrary code. This can be a security risk if you're not careful. This warning emphasizes `pixxFlow` should only be used in a **development** environment and not in a production setting where security is critical.
- **Options**
  - **comment**: _boolean_
